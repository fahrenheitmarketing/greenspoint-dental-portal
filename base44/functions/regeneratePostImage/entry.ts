...
export default async function (req) {
  try {
    ...
    const prompt = `A welcoming, bright, patient-focused photo for a ${post.platform} dental practice social media post about "${post.topic}". Style: warm natural lighting, smiling patients or friendly staff, modern clean dental office ambiance. ${brandGuide} Absolutely no scary dental tools, no clinical/surgical shots, no text overlaid on the image.${instruction ? ` Additional instruction: ${instruction}` : ''}`;
