...
export default async function (req) {
  try {
    ...
    const prompt = `A welcoming, bright, lifestyle photo for a ${post.platform} dental practice social media post about "${post.topic}". Style: warm natural lighting, happy Hispanic Latino people smiling outdoors or in everyday settings, nature, fresh healthy food, clean abstract textures. When people are shown, feature Hispanic/Latino individuals reflecting the local community. ${brandGuide} Absolutely no dental staff, no clinic reception areas, no dentist offices, no dental chairs, no scary dental tools, no clinical/surgical shots, no text overlaid on the image.${instruction ? ` Additional instruction: ${instruction}` : ''}`;
