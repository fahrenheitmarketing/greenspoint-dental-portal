          ...
          if (routing.action === 'approve_publish' || routing.action === 'approve_schedule') {
            ...
            try {
              const imgRes = await base44.asServiceRole.integrations.Core.GenerateImage({
                prompt: `${post.brand_compliance_notes ? `Visual direction from creative brief: ${post.brand_compliance_notes}. ` : ''}A welcoming, bright, lifestyle photo for a ${post.platform} dental practice post about "${post.topic}". Create a visually distinct image — vary the subject, composition, and color palette. Consider close-ups of happy Hispanic/Latino people smiling, flat-lays of fresh food, wide nature scenes, clean abstract textures, minimalist everyday-object shots, or Hispanic/Latino family/community events. ${brandGuide} When people are shown, feature Hispanic/Latino individuals reflecting the local community. No dental staff, no clinic reception areas, no dentist offices, no dental chairs, no scary tools, no clinical shots, no text in image. Adjust per: ${routing.image_instruction}`,
              });
